...
const StudentCourseQA: React.FC = () => {
    // Get course ID from params
    const { courseId } = useParams<{ courseId: string }>();
    const numericCourseId = parseInt(courseId || '1');

    // Get questions from API hook
    const { data: questionsData, isLoading, error } = useCourseQA(numericCourseId);

    const questions = (questionsData || []).map((q: any) => ({
        id: q.id,
        question: q.question || q.title,
        askedBy: q.asked_by || q.author_name || 'Anonymous',
...
export default StudentCourseQA;
