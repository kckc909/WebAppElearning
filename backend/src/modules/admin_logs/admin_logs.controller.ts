...
export class AdminLogs_Controller {
    ...
    async getByAdminId(@Param('adminId') adminId: string) {
        if (!Number(adminId)) {
            throw new BadRequestException("adminId must be integer");
        }

        const logs = await this._service.getByAdminId(Number(adminId));
        return logs;
    }

    @Get('table/:targetTable')
    async getByTargetTable(@Param('targetTable') targetTable: string) {
        const logs = await this._service.getByTargetTable(targetTable);
        return logs;
    }
